...
- Default model: `models/yolo11l-pose.engine`
...
python -c "from ultralytics import YOLO; YOLO('yolo11l-pose.pt')"
...
```bash
python -c "from ultralytics import YOLO; YOLO('models/yolo11l-pose.pt').export(format='engine', imgsz=736, half=True, device=0)"
```

After export, place or keep the generated file here:

```text
models/yolo11l-pose.engine
```
...
3. Download or auto-fetch `yolo11l-pose.pt` from official Ultralytics sources
4. Export it to `models/yolo11l-pose.engine` if TensorRT is available
...
- 默认模型为 `models/yolo11l-pose.engine`
- 默认类别过滤固定为人物 `class 0`
...
- 默认模型：`models/yolo11l-pose.engine`
- 默认类别过滤：`0`，即只识别人
...
3. 从官方 Ultralytics 来源获取 `yolo11l-pose.pt`
4. 如果本机具备 TensorRT 环境，再导出成 `models/yolo11l-pose.engine`
